...
router.post('/', jsonParser, (req, res) => {
  const requiredFields = ['email', 'password', 'establishmentName'];
  ...
  const stringFields = ['email', 'password', 'establishmentName'];
  ...
  const sizedFields = {
    ...
    password: {
      min: 8,
      // bcrypt truncates after 25 characters, so let's not give the illusion
  ...
  let {email, password, establishmentName = ''} = req.body;
